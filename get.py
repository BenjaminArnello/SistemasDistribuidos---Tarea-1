...
def get_data(tiempos, nombres):
    ...
    while i < n:
        ...
        t = threading.Thread(target=send_request, args=(url, payload, start_time, tiempos, name, numero))
        thread_list.append(t)
        t.start()
        time.sleep(0.05)  # sleep for 1 milliseconds before making another API request
...
def send_request(url, payload, start_time, tiempos, name, numero):
    ...
    elapsed_time = round((end_time - start_time) * 1000, 2)  # calculate elapsed time in milliseconds and round to 2 decimal places
    tiempos.append(elapsed_time)


   # if response.status_code == 200:
    #    print(f"{jsonified_data} (elapsed time: {elapsed_time} ms)")
    #else: print(f"failed request (elapsed time: {elapsed_time} ms)")
